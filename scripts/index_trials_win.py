...
EMBEDDING_DIMENSION = 3072  # Gemini embedding-001 max dims (normalized, best accuracy)
...
def initialize_gemini():
    ...
    if not GEMINI_API_KEY:
        print("[WARNING]  Warning: GEMINI_API_KEY not found in .env.local")
    ...
    try:
        ...
        print(f"[WARNING]  Warning: Failed to initialize Gemini API: {e}")
...
def generate_embeddings_batch(texts: List[str]) -> List[List[float]]:
    ...
    Returns list of 3072-dimensional vectors.
    """
    try:
        # Use gemini-embedding-001 model (3072 dimensions, normalized)
        embeddings = []
        ...
        for text in texts:
            ...
            result = genai.embed_content(
                ...
                # Default 3072 dims (normalized, best accuracy for semantic search)
            )
        ...
        print(f"[WARNING]  Warning: Failed to generate embeddings: {e}")
...
def calculate_quality_score(trial: Dict[str, Any]) -> float:
    ...
    try:
        enrollment_raw = trial.get('enrollment', 0)
        if isinstance(enrollment_raw, str):
            if enrollment_raw.lower() in ['none', 'na', '']:
                enrollment = 0
            else:
                enrollment = int(enrollment_raw)
        else:
            enrollment = int(enrollment_raw) if enrollment_raw else 0
...
def generate_searchable_text(trial: Dict[str, Any]) -> str:
    ...
    if trial.get('keywords'):
        keywords_list = trial['keywords']
        if isinstance(keywords_list, list):
            # Extract text from keyword objects (use 'name' key) or use as-is if strings
            keywords_text = ', '.join([
                k.get('name', '') if isinstance(k, dict) else str(k)
                for k in keywords_list
                if (isinstance(k, dict) and k.get('name') and k.get('name') != 'NA') or (isinstance(k, str) and k != 'NA')
            ])
        else:
...
def transform_trial_for_indexing(trial: Dict[str, Any], embedding: List[float] = None) -> Dict[str, Any]:
    ...
    """
    # Parse enrollment - handle string "None" and convert to integer
    enrollment = trial.get('enrollment')
    if enrollment:
        if isinstance(enrollment, str):
            if enrollment.lower() == 'none' or enrollment == 'NA':
                enrollment = None
            else:
                try:
                    ...
                    enrollment = None
        elif not isinstance(enrollment, int):
            try:
                enrollment = int(enrollment)
            except (ValueError, TypeError):
                enrollment = None

    # Convert keywords list to text string
    keywords_text = ''
    if trial.get('keywords'):
        keywords_list = trial['keywords']
        if isinstance(keywords_list, list):
            # Extract text from keyword objects (use 'name' or 'downcase_name' key) or use as-is if strings
            keywords_parts = []
            for k in keywords_list:
                if isinstance(k, dict):
                    # Try 'name', 'downcase_name', or 'keyword' keys
                    keyword_val = k.get('name') or k.get('downcase_name') or k.get('keyword', '')
                    if keyword_val and keyword_val != 'NA':
                        keywords_parts.append(str(keyword_val))
                elif isinstance(k, str) and k != 'NA':
                    keywords_parts.append(k)
    ...
    doc = {
        ...
        'keywords': keywords_text,  # Convert to text string
...
def create_index_if_not_exists(es: Elasticsearch):
    ...
    print(f"[OK] Index '{INDEX_NAME}' created with dense_vector field ({EMBEDDING_DIMENSION} dims, cosine similarity)")
...
def bulk_index_trials(es: Elasticsearch, trials: List[Dict], use_embeddings: bool = True):
    ...
    if use_embeddings:
        ...
        print(f"[OK] Generated {len(embeddings)} embeddings ({EMBEDDING_DIMENSION} dimensions each)")
    else:
        print("[WARNING]  Skipping embedding generation (Gemini API not available)")
    ...
    print(f"[DEBUG] Prepared {len(actions)} actions, {embedded_count} with embeddings")
    ...
    with tqdm(total=len(actions), desc="Indexing progress") as pbar:
        for ok, response in helpers.streaming_bulk(
            ...
            if ok:
                ...
                failed += 1
                print(f"\n[WARNING]  Failed to index document: {response}")
            pbar.update(1)

    print(f"\n[OK] Successfully indexed {success} trials")
    if failed > 0:
        print(f"[WARNING]  Failed to index {failed} trials")
...
def validate_indexing(es: Elasticsearch):
    ...
    print(f"[OK] Index health: {health['status'].upper()}")
    ...
    if search_result['hits']['total']['value'] > 0:
        print("[OK] Search test successful")
        sample_doc = search_result['hits']['hits'][0]
        print(f"[OK] Sample document: {sample_doc['_source']['nct_id']} - {sample_doc['_source']['brief_title'][:60]}...")

        # Check if embeddings are present
        if 'description_embedding' in sample_doc['_source']:
            print(f"[OK] Embeddings present ({len(sample_doc['_source']['description_embedding'])} dimensions)")
        else:
            print("[WARNING]  No embeddings found in documents")
...
def main():
    ...
    with open(DATASET_PATH, 'r', encoding='utf-8') as f:
        trials = json.load(f)
    print(f"[OK] Loaded {len(trials)} trials")

    # Check Elasticsearch credentials
    if not ES_CLOUD_ID and not ES_API_KEY:
        print("\n[ERROR] Error: Elasticsearch credentials not found!")
    ...
    try:
        if ES_CLOUD_ID and ES_API_KEY:
            ...
            print("[ERROR] Error: No valid authentication method found")
            sys.exit(1)

        # Test connection
        info = es.info()
        print(f"[OK] Connected to Elasticsearch {info['version']['number']}")
        print(f"[OK] Cluster: {info['cluster_name']}")
    except Exception as e:
        print(f"[ERROR] Error connecting to Elasticsearch: {e}")
        print("\nTroubleshooting:")
    ...
    print("\n[OK] Your clinical trial intelligence platform is ready!")
